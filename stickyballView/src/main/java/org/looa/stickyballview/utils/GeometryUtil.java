...
public class GeometryUtil {
    ...
    public static PointF[] getIntersectionPoints(PointF o1, PointF o2, float r1, float r2, double k) {
        ...
        if (!Double.isNaN(k) && !Double.isInfinite(k)) {
            ...
            if (o1.y - o2.y > 0) p1.set(o1.x - dX1, o1.y - dY1);
            else p1.set(o1.x - dX1, o1.y + dY1);
        ...
        if (!Double.isNaN(k) && !Double.isInfinite(k)) {
            ...
            if (o1.y - o2.y > 0) p2.set(o2.x - dX2, o2.y + dY2);
            else p2.set(o2.x - dX2, o2.y - dY2);

        }
        //以o2为原点的p1、p2相对坐标值
        float[] p1XY = {p1.x - o2.x, p1.y - o2.y};
        float[] p2XY = {p2.x - o2.x, p2.y - o2.y};
        double a = k, b = -1;
        //以o2为原点的p3、p4的相对坐标
        float p3X, p3Y, p4X, p4Y;
        if (!Double.isNaN(k) && !Double.isInfinite(k)) {
            p3X = (float) (p1XY[0] - 2 * a * (a * p1XY[0] + b * p1XY[1]) / (Math.pow(a, 2) + Math.pow(b, 2)));
            p3Y = (float) (p1XY[1] - 2 * b * (a * p1XY[0] + b * p1XY[1]) / (Math.pow(a, 2) + Math.pow(b, 2)));
            p4X = (float) (p2XY[0] - 2 * a * (a * p2XY[0] + b * p2XY[1]) / (Math.pow(a, 2) + Math.pow(b, 2)));
            p4Y = (float) (p2XY[1] - 2 * b * (a * p2XY[0] + b * p2XY[1]) / (Math.pow(a, 2) + Math.pow(b, 2)));
        } else {
            p3X = -p1XY[0];
            p3Y = p1XY[1];
            p4X = -p2XY[0];
