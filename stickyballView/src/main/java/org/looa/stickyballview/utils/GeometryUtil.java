...
public class GeometryUtil {
    ...
    public static PointF[] getIntersectionPoints(PointF o1, PointF o2, float r1, float r2, double k) {
        ...
        double a = k, b = -1;
        //以o2为原点的p3、p4的相对坐标
        float p3X = (float) (p1XY[0] - 2 * a * (a * p1XY[0] + b * p1XY[1]) / (Math.pow(a, 2) + Math.pow(b, 2)));
        float p3Y = (float) (p1XY[1] - 2 * b * (a * p1XY[0] + b * p1XY[1]) / (Math.pow(a, 2) + Math.pow(b, 2)));
        float p4X = (float) (p2XY[0] - 2 * a * (a * p2XY[0] + b * p2XY[1]) / (Math.pow(a, 2) + Math.pow(b, 2)));
        float p4Y = (float) (p2XY[1] - 2 * b * (a * p2XY[0] + b * p2XY[1]) / (Math.pow(a, 2) + Math.pow(b, 2)));
